...
export function ResultsVerificationStepper({
  verifiedAt,
  approvedCount,
  requiredCount,
  publishedAt,
  publishedBy,
}: ResultsVerificationStepperProps) {
  ...
  const steps: StepperStep[] = [
    { label: "Counting Completed", completed: !!verifiedAt },
    {
      ...
      timestamp: verifiedAt ? formatStepTime(verifiedAt) : undefined,
    },
    {
      label: "DAT Approved",
      completed: datComplete,
      detail: `${approvedCount}/${requiredCount} Approvals`,
    },
    {
      label: "Published",
      completed: true,
      detail: publishedBy ? `by ${publishedBy}` : undefined,
      timestamp: formatStepTime(publishedAt),
    },
  ];
        ...
        {steps.map((step, i) => (
          <div key={step.label} className="flex flex-1 items-start">
            <div className="flex min-w-0 flex-1 flex-col items-center text-center">
              ...
              <p className="mt-2 text-xs font-bold text-[var(--sevs-navy)]">
                {step.label}
              </p>
              {step.detail && (
                <p className="text-[11px] text-[var(--sevs-text-muted)]">
                  {step.detail}
                </p>
              )}
              {step.timestamp && (
            ...
            {i < steps.length - 1 && (
              <div
                className={`mt-4 h-0.5 flex-1 ${step.completed ? "bg-emerald-500" : "bg-gray-200"}`}
              />
            )}
          </div>
        ))}
        ...
        {steps.map((step, i) => (
          <div key={step.label} className="flex gap-3">
            <div className="flex flex-col items-center">
              ...
              {i < steps.length - 1 && (
                <div
                  className={`mt-1 w-0.5 flex-1 ${step.completed ? "bg-emerald-500" : "bg-gray-200"}`}
                  style={{ minHeight: 24 }}
                />
              )}
            </div>
