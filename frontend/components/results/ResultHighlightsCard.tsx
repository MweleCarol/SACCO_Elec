...
export function ResultHighlightsCard({ highlights }: { highlights: ResultHighlight[] }) {
  return (
    <div className="rounded-2xl border border-[var(--sevs-border)] bg-white p-5 shadow-sm sm:p-6">
      <div className="mb-4 flex items-center gap-2">
        <Lightbulb className="h-4 w-4 text-amber-500" />
        <h3 className="text-sm font-bold uppercase tracking-wide text-[var(--sevs-text-muted)]">Key Highlights</h3>
      </div>
      <ul className="space-y-2 text-sm text-[var(--sevs-text-body)]">
        {highlights.map((h) => (
          <li key={h.label} className="flex gap-1.5">
            ...
            <span>
              <span className="font-semibold text-[var(--sevs-navy)]">{h.label}</span>
              {h.value && `: ${h.value}`}
            </span>
          </li>
        ))}
