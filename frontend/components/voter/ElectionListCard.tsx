    ...
    <div className="rounded-2xl border border-[var(--sevs-border)] bg-white p-4 shadow-sm sm:p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="break-words text-base font-bold leading-snug text-[var(--sevs-navy)]">
            {election.title}
          </h3>
          <p className="mt-1 line-clamp-2 break-words text-sm text-[var(--sevs-text-muted)]">
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        <p className="min-w-0 truncate text-xs text-[var(--sevs-text-muted)]">
          Closes{" "}
          {
            formatDateRange(election.startDate, election.endDate).split(
              " – ",
            )[1]
          }
        </p>
