...
export function ElectionOfficerDashboard({
  user,
}: ElectionOfficerDashboardProps) {
  const activeElections = mockElections.filter(
    (e) => getComputedLifecycleStatus(e) === "ACTIVE",
  );
  const approvedCandidates = mockCandidates.filter(
    (c) => c.status === "APPROVED",
  ).length;
  ...
  const avgTurnout =
    activeElections.length > 0
      ? Math.round(
          activeElections.reduce((sum, e) => sum + getVotingProgress(e), 0) /
            activeElections.length,
        )
      : 0;

  const pendingApprovalItems = mockApprovals
    ...
    <div className="space-y-6 p-8">
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint="Approved"
          icon={<Users2 className="h-4 w-4 text-[var(--sevs-navy)]" />}
        />
        <StatCard
        ...
        <StatCard
          label="Avg. Turnout"
          value={`${avgTurnout}%`}
          hint="Across active elections"
          icon={<TrendingUp className="h-4 w-4 text-[var(--sevs-navy)]" />}
        />
      </div>
      ...
      <div className="rounded-2xl border border-[var(--sevs-border)] bg-white p-6 shadow-sm">
        ...
        <div className="space-y-3 sm:hidden">
          {mockElections.map((election) => {
            const candidateCount = getCandidatesByElection(election.id).length;
            const isReview =
            ...
            return (
              <div
                key={election.id}
                className="min-w-0 rounded-xl border border-[var(--sevs-border)] p-4"
              >
                <div className="flex flex-col gap-2">
                  <span className="break-words font-medium text-[var(--sevs-navy)]">
                    {election.title}
                  </span>
                  <div>
                ...
                <div className="mt-2 text-sm text-[var(--sevs-text-body)]">
          ...
          <div className="space-y-3 text-sm">
            {pendingApprovalItems.map((a) => (
              <div key={a.id} className="flex items-center justify-between">
                <span className="text-[var(--sevs-text-body)]">
                  {a.targetLabel}
                </span>
